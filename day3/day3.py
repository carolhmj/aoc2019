...
segments1 = create_segments(line1)
segments2 = create_segments(line2)

print (segments1, segments2)

# Compute intersections between each pair of segments
intersections_by_distance = []
intersections_by_steps = []

for (p1, p2) in itertools.product(segments1, segments2):
    segment1 = p1[0]
    segment2 = p2[0]

    maybe_intersection = segment1.intersection(segment2)            
    if len(maybe_intersection) > 0:
        print (maybe_intersection)
        if isinstance(maybe_intersection[0], Segment):
            intersection = maybe_intersection[0].p1
        else:
            intersection = maybe_intersection[0]
        
        distance = abs(intersection[0]) + abs(intersection[1])
        steps1 = p1[1] + Segment(segment1.p1, intersection).length
        steps2 = p2[1] + Segment(segment2.p1, intersection).length
        steps = steps1 + steps2

        heapq.heappush(intersections_by_distance, 
        (distance, intersection))
        heapq.heappush(intersections_by_steps, (steps, intersection))

print (intersections_by_steps)

# Pop the smallest intersection, which we know it's (0,0)
heapq.heappop(intersections_by_distance)
heapq.heappop(intersections_by_steps)

# Pop the second smallest one, i.e. the closest
print ('by distance', intersections_by_distance[0])
print ('by steps', intersections_by_steps[0])
